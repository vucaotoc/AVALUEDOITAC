...
@WebServlet(name = "sl_themhssobo", urlPatterns = {"/themsobo"})
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 2, // 2MB
        maxFileSize = 1024 * 1024 * 100, // 50MB
        maxRequestSize = 1024 * 1024 * 100) // 50MB
public class sl_themhssobo extends HttpServlet {
                ...
                for (Part part : request.getParts()) {
                    ...
                    if (fileName.length() > 0) {
                        if (!Arrays.asList(DATA.function._fileBlock).contains(duoifile)) {
                            ...
                            String tenfile = "DTHSSB_" + iddths + "_" + obj.getIddt() + "_" + calendar.getTimeInMillis() + "_" + dem + "." + duoifile;
                            dsfile += tenfile + ";";
